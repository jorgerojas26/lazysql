...
func TestMySQL_ExecuteQuery_CrossDatabase(t *testing.T) {
	...
	results, total, err := mysql.ExecuteQuery(context.Background(), "targetdb", "SELECT * FROM users")
	if err != nil {
		t.Fatalf("ExecuteQuery failed: %v", err)
	}
...
func TestMySQL_ExecuteQuery_SameDatabase_NoUseStatement(t *testing.T) {
	...
	_, _, err = mysql.ExecuteQuery(context.Background(), "targetdb", "SELECT * FROM users")
	if err != nil {
		t.Fatalf("ExecuteQuery failed: %v", err)
	}
