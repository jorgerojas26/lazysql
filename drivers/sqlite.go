...
func (db *SQLite) GetReferencingTables(ctx context.Context, database, table string) ([][]string, error) {
	if table == "" {
		return nil, errors.New("table name is required")
	}

	rows, err := db.Connection.QueryContext(contextOrBackground(ctx), `
        SELECT
	...
	return db.resolveImplicitReferencedColumns(ctx, database, table, results)
}
...
func (db *SQLite) resolveImplicitReferencedColumns(ctx context.Context, database, table string, results [][]string) ([][]string, error) {
	...
	primaryKeyColumnNames, err := db.GetPrimaryKeyColumnNames(ctx, database, table)
	if err != nil {
		return nil, err
	}
