...
func (db *MySQL) ExecutePendingChanges(changes []models.DbDmlChange) (err error) {
	...
	for _, change := range changes {
		columns := make([]string, len(change.Values))
		values := make([]interface{}, len(change.Values))
		valuesPlaceholder := make([]string, len(change.Values))
		...
		case models.DmlUpdateType:
			...
			args := make([]interface{}, len(change.Values)+1)
